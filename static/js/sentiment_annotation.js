...
window.Sentiment = {
    ...
    load_data: function (bundle_id, sentence_id, add_to_word_connections) {
        ...
        if (add_to_word_connections == null) add_to_word_connections = true;

        var sent_ord = sentence_id_to_order[sentence_id];
        if (sent_ord > 0) {
            $.when(window.Sentiment.load_data(bundle_id, sentence_order[sent_ord - 1], false)).then(function () {
    ...
    _request_and_add: function (bundle_id, sentence_id, add_to_word_connections) {
        var req_data = {
            "bundle_id": bundle_id, "sentence_id": sentence_id, "username": $("#username").val()
        };
        var delayed = [];
        var jreq = $.ajax({
            type: 'GET', url: "/grapat", data: req_data, dataType: "json", async: false, success: function (data) {
                ...
                if (add_to_word_connections) {
                    if (graph != null) loaded_annotations = graph;
                }
                ...
                if (graph != null) {
                        ...
                        $.each(value, function (target_id, edges) {
                            $.each(edges, function (conn_id, attrs) {
                                ...
                                if ($('#' + target_id)[0] == null || $('#' + source_id)[0] == null) {
                                    ...
                                    } else delayed.push([source_id, target_id, attrs]);
                                    // jquery continue
                                    return true;
                                }
                                if (!add_to_word_connections && (source_id.indexOf("word_") == 0 || target_id.indexOf("word_") == 0)) {
                                    ...
                                    if (current_connection.source.nodeName != "SPAN") {
                                        if (annotation_type == 'sentiment') window.Sentiment.labelPopUpButton_click(attrs.label_node_id, attrs.polarity, attrs.text_anchor, attrs.context, attrs.world_knowledge, attrs.ironic, attrs.rhetoric); else if (annotation_type == 'argumentation') {
                    ...
                    while (delayed.length > 0) {
                        ...
                        if ($('#' + target_id)[0] == null || $('#' + source_id)[0] == null) {
                            ...
                            // js continue
                            if (it_count < 1000) continue; else alert('Loading error. Your saved data is fine! Please just reload the sentence.');
                        }
                        current_connection = jsPlumb.connect({source: source_id, target: target_id});
                        current_source = current_connection.sourceId;
                        current_target = current_connection.targetId;
                        if (current_connection.source.nodeName != "SPAN") window.Sentiment.labelPopUpButton_click(attrs.label_node_id, attrs.polarity, attrs.text_anchor, attrs.context, attrs.world_knowledge, attrs.ironic, attrs.rhetoric);
    ...
    add_node: function (node_id, x, y, label, type) {
        ...
        if ($('#' + node_id).length == 0) {
            ...
            var new_node = jQuery('<div/>', {
                class: 'window movable invisible node deletable', id: node_id, node_id: node_count,
            });
            ...
            if (type == 'circle') new_node.addClass("circle");
            ...
            else if (type == 'node_type_mixed')
                new_node.addClass("node_type_mixed");
            ...
            jsPlumb.makeSource(source_handle, {
                parent: new_node, anchor: "Continuous", connectorStyle: {
...
//            jsPlumb.addEndpoint($("#" + node_id), ent_endpoints);
    ...
    get_files_to_be_annotated: function () {
        $.getJSON("/resources", function (data) {
            ...
            window.Sentiment.change_annot_file();
    ...
    add_word: function (to_add, wid, trid) {
        if ($('#word_' + wid).length == 0) {
            ...
            jQuery('<span/>', {
                class: word_type + ' window', id: 'word_' + wid, text: to_add, token_range_id: trid
            }).appendTo($("#sentence"));
    ...
    next_sentence: function (sa) {
        ...
        if (sa && changed) window.Sentiment.save();
    ...
    save: function () {
        ...
        var nodes = $(".node");
        $.each(nodes, function () {
            if (!($(this)[0].id in layout)) layout[$(this)[0].id] = {};
    ...
    downloadFile: function () {
        $.get('/grapat/export', {
            ...
            dlink.download = annotation_bundle_id + '.xml';
    ...
    init_arg: function () {
        ...
        jQuery('<div/>', {
            class: 'rmenu_element', id: 'add_proponent_node', text: 'add ADU proponent'
        }).appendTo('#rmenu');
        jQuery('<div/>', {
            class: 'rmenu_element', id: 'add_opponent_node', text: 'add ADU opponent'
        }).appendTo('#rmenu');
        jQuery('<div/>', {
            class: 'rmenu_element', id: 'add_mixed_node', text: 'add ADU mixed'
        }).appendTo('#rmenu');
        jQuery('<div/>', {
            class: 'rmenu_element', id: 'add_join_edus', text: 'add EDU join'
        }).appendTo('#rmenu');

        // bind functions to the rmenu entries
        $("#add_opponent_node").bind("click", function () {
        ...
        $("#add_mixed_node").bind("click", function () {
            var node_id = 'node_' + node_count;
            annotations.nodes[node_id] = {};
            window.Sentiment.add_node(node_id, rclick.pageX, rclick.pageY, 'new node', 'node_type_mixed');
        });

        $("#add_proponent_node").bind("click", function () {
            var node_id = 'node_' + node_count;
            annotations.nodes[node_id] = {};
    ...
    init_sent: function () {
        jQuery('<div/>', {
            class: 'rmenu_element', id: 'add_ent', text: 'add entity'
        }).appendTo('#rmenu');
        jQuery('<div/>', {
            class: 'rmenu_element', id: 'del_ele', text: 'delete element'
        }).appendTo('#rmenu');
        ...
        jQuery('<div/>', {
            class: 'extra_sentential_node window', id: 'author', node_id: 'node_0', text: 'Autor'
        }).appendTo('#graph_part');
    ...
    read_input_file: function (filename) {
        ...
        jQuery.get('resources/' + filename, function (data) {
            ...
            if (annotation_type == 'argumentation') window.Sentiment.init_arg(); else if (annotation_type == 'sentiment') window.Sentiment.init_sent();
    ...
    update: function () {
        ...
        var sourceAnchors;
        sourceAnchors = [[0, 1, 0, 1], [0.25, 1, 0, 1], [0.5, 1, 0, 1], [0.75, 1, 0, 1], [1, 1, 0, 1], "TopCenter", "RightMiddle", "LeftMiddle"];
        ...
        jsPlumb.importDefaults({
            ...
            Anchors: [sourceAnchors], Connector: ["Bezier", {curviness: 50}], // drag options
            DragOptions: {cursor: "pointer", zIndex: 2000}, // default to blue at source and green at target
            EndpointStyles: [{fillStyle: "black"}, {fillStyle: "#black"}], // blue endpoints 7 px; green endpoints 11.
            Endpoints: [["Dot", {radius: 2}], ["Dot", {radius: 2}]], // default to a gradient stroke from blue to green.  for IE provide all green fallback.
            PaintStyle: {
                //gradient:{ stops:[ [ 0, "black" ], [ 1, "black" ] ] },
                strokeStyle: "black", lineWidth: 1.5
            }, hoverPaintStyle: {
                strokeStyle: "black", lineWidth: 5
...
//	    };
        ...
        jsPlumb.makeSource(wordDivs, {
            anchor: ["TopCenter"], endpoint: ["Dot", {radius: 0}], paintStyle: {
                lineWidth: 0
            }
        });
    ...
    labelPopUpButton_click: function (label_node_id, pol, text, ctxt, wk, irn, rhtrc, c_t) {
        ...
        if (annotations.edges[current_source][current_target][current_connection.id]["label_node_id"] != null) old = true;
        ...
        if (annotation_type === 'sentiment') {
            if (pol != null) polarity = pol; else polarity = $('input[name="polarity"]:checked').val();

            if (ctxt != null) context = ctxt; else context = $('input[name="context"]:checked').val();

            if (wk != null) worldknowledge = wk; else worldknowledge = $('input[name="wknow"]:checked').val();

            if (irn != null) irony = irn; else irony = $('input[name="ironic"]:checked').val();

            if (rhtrc != null) rhetoric = rhtrc; else rhetoric = $('input[name="rhetoric"]:checked').val();
        ...
        if (polarity == 'negative' || polarity == 'positive') {
            ...
            current_connection.addOverlay(["Custom", {
                create: function (component) {
                    return $('<div></div>');
                }, location: 1, cssClass: "circle rebut_undercut_circle"
            }]);
        }

        // add edge node as overlay
        var arc_label = text_anchor;
        if (c_type == 'rebut') arc_label = 'rebut';
        if (c_type == 'undercut') arc_label = 'undercut';
        if (c_type == 'support') arc_label = 'support';
        if (c_type == 'support_by_example') arc_label = 'example';

        if (c_type != 'additional_source') {
            current_connection.addOverlay(["Custom", {
                create: function (component) {
                    return $('<div id="' + ln_id + '" class="edge_label target">' + arc_label + '</div>');
                }, location: 0.5, cssClass: "edge_label target", id: "labelNode"					//(["Label", {label: text_anchor, id: "label", cssClass: "edge_label target"}]);
            }]);
        } else {
            // remove labelnode id
        ...
        if (!old && c_type != 'additional_source') ++node_count;
    ...
    showAttrsPopUp: function (c) {
        if (c.source.nodeName == "SPAN") return false;
    ...
    connection_sent: function (i, c) {
        ...
        if (i.connection.sourceId in annotations.edges && i.connection.targetId in annotations.edges[i.connection.sourceId] && i.connection.id in annotations.edges[i.connection.sourceId][i.connection.targetId]) return;

        // of the form "edges": {"node_0": {"node_1": "node_0_to_1_weight"}, ...}
        if (!(i.connection.sourceId in annotations.edges)) annotations.edges[i.connection.sourceId] = {};
        if (!(i.connection.targetId in annotations.edges[i.connection.sourceId])) annotations.edges[i.connection.sourceId][i.connection.targetId] = {};
        //var sentence_id = sentence_order[current_sentence_idx];
        ...
        if (i.connection.source.nodeName != 'SPAN') i.connection.toggleType('default');
        ...
        if (i.connection.source.nodeName == 'SPAN' && (target_span.innerText == 'new node' || target_span.innerHTML == 'new node')) {
            if (add_to_node_text) target_span.innerHTML = i.connection.source.innerHTML; else target_span.innerHTML = alt_node_text;
        ...
        if (i.connection.source.nodeName == 'SPAN' && target_span.innerHTML.indexOf(i.connection.source.innerHTML) < 0) {
            if (add_to_node_text) target_span.innerHTML += ";" + i.connection.source.innerHTML;
            //annotations.nodes[i.connection.targetId] += ";" + i.connection.source.innerHTML;
        }
        ...
        if (i.connection.targetId in annotations.nodes) {
            if (annotations.nodes[i.connection.targetId]['label'] == "" && i.connection.source.nodeName == "SPAN") { //TODO: unclear what to do with the first condition
                if (add_to_node_text) annotations.nodes[i.connection.targetId]['label'] = i.connection.source.innerHTML;
                else annotations.nodes[i.connection.targetId]['label'] = alt_node_text;
        ...
        var loading = false;
        if (c == null) loading = true;
        if (!loading) window.Sentiment.showAttrsPopUp(i.connection);
    },
    ...
    _get_arg_semantic_type_of_element: function (e) {
        ...
        else if (e.classList.contains('node_type_mixed')) return "ADU";
        else return "unknown";
    },
    ...
    _get_arg_semantic_role_of_element: function (e) {
        ...
        else if (e.classList.contains('node_type_mixed')) return "mixed";
        else return null;
    },
    ...
    _modal_choice_dialog: function (message, option1, option2, connection) {
        bootbox.dialog({
            "message": message, "closeButton": false, "buttons": {
                choice1: {
                    "label": option1, "className": "btn-primary", "callback": function () {
                        window.Sentiment.labelPopUpButton_click(null, null, null, null, null, null, null, option1);
                    }
                }, choice2: {
                    "label": option2, "className": "btn-primary", "callback": function () {
    ...
    connection_arg: function (i, c) {
        ...
        if (!(i.connection.targetId in annotations.edges[i.connection.sourceId])) annotations.edges[i.connection.sourceId][i.connection.targetId] = {};
        annotations.edges[i.connection.sourceId][i.connection.targetId][i.connection.id] = {
            "label_node_id": null, "c_type": null
        };
